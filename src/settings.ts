...
import { scheduleSummary, getNextOccurrence } from './scheduler';
	...
	display(): void {
		...
		if (typeof Notification !== 'undefined') {
			if (Notification.permission === 'default') {
				...
				new Setting(containerEl)
					.setDesc('⚠ Notification permission denied. Notifications will only appear as in-app notices.');
			}
		}
		...
		for (const entry of sorted) {
			...
			const s = new Setting(containerEl)
				...
				.setDesc(`${summary} · ${nextStr}${entry.enabled ? '' : ' · DISABLED'}`);

			// Enable toggle
			s.addToggle(toggle => toggle
