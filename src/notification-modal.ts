...
export class NotificationModal extends Modal {
	...
	constructor(app: App, entry: NotificationEntry | null, onSave: (entry: NotificationEntry) => void) {
		super(app);
	...
	onOpen(): void {
		...
		new Setting(contentEl)
			...
			.setDesc("Optional. Clicking the notification opens this file.")
			.addText((text) => {
				text.setPlaceholder("folder/note.md")
					.setValue(this.entry.linkedNote ?? "")
					.onChange((v) => {
						this.entry.linkedNote = v || undefined;
					});
				new FileSuggest(this.app, text.inputEl);
			});

		// Schedule type
		new Setting(contentEl).setName("Schedule").addDropdown((dd) =>
			dd
				.addOption("once", "Once")
		...
		// Time — native OS time picker
		new Setting(contentEl).setName("Time").addText((text) => {
			text.setValue(this.entry.time).onChange((v) => {
				this.entry.time = v;
			});
		...
		if (this.entry.scheduleType !== "once") {
			new Setting(contentEl)
				...
				.setDesc("Optional")
				.addText((text) => {
					text.setValue(this.entry.startDate ?? "").onChange((v) => {
						this.entry.startDate = v || undefined;
					});
			...
			new Setting(contentEl)
				...
				.setDesc("Optional")
				.addText((text) => {
					text.setValue(this.entry.endDate ?? "").onChange((v) => {
						this.entry.endDate = v || undefined;
					});
					text.inputEl.type = "date";
				});
		}
		...
		new Setting(contentEl)
			...
			.addDropdown((dd) => {
				...
				const currentVal =
					typeof this.entry.duration === "number" ? String(this.entry.duration) : this.entry.duration;
		...
		new Setting(contentEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.entry.sound).onChange((v) => {
					this.entry.sound = v;
				}),
			);
		...
		new Setting(contentEl).setName("Enabled").addToggle((toggle) =>
			toggle.setValue(this.entry.enabled).onChange((v) => {
				this.entry.enabled = v;
			}),
		);
		...
		buttonRow.addButton((btn) => btn.setButtonText("Test").onClick(() => this.testNotification()));
		...
		switch (this.entry.scheduleType) {
			case "once":
				new Setting(container).setName("Date").addText((text) => {
					text.setValue(this.entry.date ?? "").onChange((v) => {
						this.entry.date = v || undefined;
					});
					text.inputEl.type = "date";
				});
				break;
			...
			case "weekly": {
				const setting = new Setting(container).setName("Days of week");
			...
			case "monthly":
				new Setting(container).setName("Day of month").addDropdown((dd) => {
					for (let i = 1; i <= 31; i++) {
						dd.addOption(String(i), String(i));
					}
					dd.setValue(String(this.entry.dayOfMonth ?? 1));
					dd.onChange((v) => {
						this.entry.dayOfMonth = parseInt(v, 10);
					});
				});
				break;

			case "custom":
				new Setting(container).setName("Interval (days)").addText((text) =>
					text
						.setPlaceholder("7")
				...
				new Setting(container)
					.setName("Start from")
					.setDesc("Anchor date for interval")
					.addText((text) => {
						text.setValue(this.entry.customIntervalStartDate ?? "").onChange((v) => {
							this.entry.customIntervalStartDate = v || undefined;
						});
						text.inputEl.type = "date";
					});
				break;
